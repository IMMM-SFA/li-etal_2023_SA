...
load('gamma_summer.mat','lambdaminus1_diff_frac_summer','sig_matrix_summer','mask_p_AC_summer');
load('mu_data.mat')
load('mask_urban_and_rural.mat')

%% Figure 2a

plot_value = lambdaminus1_diff_frac_summer;
...
% sig_matrix_relocation = sig_matrix_summer;
...
values_get_size = size(mask_p_AC_summer);
mask_p_AC_relocation(1:values_get_size(1)/2,:) = mask_p_AC_summer((values_get_size(1)/2+1):values_get_size(1),:); % flip left-right
mask_p_AC_relocation((values_get_size(1)/2+1):values_get_size(1),:) = mask_p_AC_summer(1:values_get_size(1)/2,:);
...
print(gcf,'-dpng','-r600','Spatial_autocorrelation_difference_fraction_summer')
...
% x = mu(:)/1e4;
% y = lambdaminus1_diff_frac_summer(:);
% 
...
% end
% 
% x_sig_temp = mu/1e4.*mask_p_AC_summer;
% y_sig_temp = lambdaminus1_diff_frac_summer.*mask_p_AC_summer;
% 
% x_sig = x_sig_temp(:);
% y_sig = y_sig_temp(:);
% 
...
% print(gcf,'-dpng','-r600','mu_and_gamma_summer')
