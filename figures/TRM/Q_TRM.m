...
% sensitivity of surface saturated specific humidity to albedo
BB = ra_ref./(ra_ref+rs_ref).*dqs_sat_dTs_ref;
dqs_dalpha_TRM_ref = BB.*dTs_dalpha_TRM_ref;

% sensitivity of surface saturated specific humidity to aerodynamic resistance
dqs_dra_TRM_ref = rs_ref./(ra_ref+rs_ref).^2.*(qs_sat_modeled_ref-qa_ref)+BB.*dTs_dra_TRM_ref;

% sensitivity of surface saturated specific humidity to surface resistance
dqs_drs_TRM_ref = -ra_ref./(ra_ref+rs_ref).^2.*(qs_sat_modeled_ref-qa_ref)+BB.*dTs_drs_TRM_ref;

% sensitivity of surface saturated specific humidity to ground heat flux
