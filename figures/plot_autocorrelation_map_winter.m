...
load('gamma_winter.mat','lambdaminus1_diff_frac_winter','sig_matrix_winter','mask_p_AC_winter');
load('mu_data.mat')
load('mask_urban_and_rural.mat')

%% Figure 2a

plot_value = lambdaminus1_diff_frac_winter;
...
% sig_matrix_relocation = sig_matrix_winter;
...
values_get_size = size(mask_p_AC_winter);
mask_p_AC_relocation(1:values_get_size(1)/2,:) = mask_p_AC_winter((values_get_size(1)/2+1):values_get_size(1),:); % flip left-right
mask_p_AC_relocation((values_get_size(1)/2+1):values_get_size(1),:) = mask_p_AC_winter(1:values_get_size(1)/2,:);
...
print(gcf,'-dpng','-r600','Spatial_autocorrelation_difference_fraction_winter')
...
% x = mu(:)/1e4;
% y = lambdaminus1_diff_frac_winter(:);
% 
...
% end
% 
% x_sig_temp = mu/1e4.*mask_p_AC_winter;
% y_sig_temp = lambdaminus1_diff_frac_winter.*mask_p_AC_winter;
% 
% x_sig = x_sig_temp(:);
% y_sig = y_sig_temp(:);
% 
...
% print(gcf,'-dpng','-r600','mu_and_gamma_winter')
