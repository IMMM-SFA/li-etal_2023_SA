...
addpath('/Users/danl/MatlabFunctions/m_map')
...
%%
% figure
% plot(AC1_urban_Ts,AC1_urban,'ro')
% hold on
% plot(AC1_rural_Ts,AC1_rural,'bo')
% hold on

%% Figure 2a

plot_value = lambdaminus1_diff_frac_Ts;
...
% sig_matrix_relocation = sig_matrix_Ts;
...
values_get_size = size(mask_p_AC_Ts);
mask_p_AC_relocation(1:values_get_size(1)/2,:) = mask_p_AC_Ts((values_get_size(1)/2+1):values_get_size(1),:); % flip left-right
mask_p_AC_relocation((values_get_size(1)/2+1):values_get_size(1),:) = mask_p_AC_Ts(1:values_get_size(1)/2,:);
...
print(gcf,'-dpng','-r600','Spatial_autocorrelation_difference_fraction_Ts')
...
% end
% 
% x_sig_temp = mu/1e4.*mask_p_AC_Ts;
% y_sig_temp = lambdaminus1_diff_frac_Ts.*mask_p_AC_Ts;
% 
% x_sig = x_sig_temp(:);
% y_sig = y_sig_temp(:);
% 
...
% print(gcf,'-dpng','-r600','mu_and_gamma_Ts')
