...
load('gamma_autumn.mat','lambdaminus1_diff_frac_autumn','sig_matrix_autumn','mask_p_AC_autumn');
load('mu_data.mat')
load('mask_urban_and_rural.mat')

%% Figure 2a

plot_value = lambdaminus1_diff_frac_autumn;
...
% sig_matrix_relocation = sig_matrix_autumn;
...
values_get_size = size(mask_p_AC_autumn);
mask_p_AC_relocation(1:values_get_size(1)/2,:) = mask_p_AC_autumn((values_get_size(1)/2+1):values_get_size(1),:); % flip left-right
mask_p_AC_relocation((values_get_size(1)/2+1):values_get_size(1),:) = mask_p_AC_autumn(1:values_get_size(1)/2,:);
...
print(gcf,'-dpng','-r600','Spatial_autocorrelation_difference_fraction_autumn')
...
% x = mu(:)/1e4;
% y = lambdaminus1_diff_frac_autumn(:);
% 
...
% end
% 
% x_sig_temp = mu/1e4.*mask_p_AC_autumn;
% y_sig_temp = lambdaminus1_diff_frac_autumn.*mask_p_AC_autumn;
% 
% x_sig = x_sig_temp(:);
% y_sig = y_sig_temp(:);
% 
...
% print(gcf,'-dpng','-r600','mu_and_gamma_autumn')
